...
int main() {
    ...
    for(int i = 0;i < suf.suffix.size();i++) {
        ll substr = (int)s.size() - suf.suffix[i] - suf.lcp[i];
        while(p < queries.size() && queries[p].first <= substr + running) {
            queries[p].first -= running;
            ans[queries[p].second] = s.substr(suf.suffix[i], suf.lcp[i] + queries[p].first);
            p++;
        }
        running += substr;
        if(p == queries.size())break;
    }
    for(auto &val:ans)
        cout<<val<<line;
}
