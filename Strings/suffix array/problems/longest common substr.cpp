...
signed main() {
    ...
    for(int i = 1;i < lcp.size();i++) {
        if(suf[i] < s.size() && suf[i + 1] >= s.size() || suf[i] >= s.size() && suf[i + 1] < s.size()) {
            if(lcp[i] > res)res = lcp[i] , idx = min(i , i + 1 , [](const int &k , const int j)->bool {
                return suf[k] < suf[j];
            });
        }
    }
    cout<<s.substr(suf[idx] , res)<<line;
}
