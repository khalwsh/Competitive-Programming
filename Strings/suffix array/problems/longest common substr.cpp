...
int main() {
    ...
    string s , t;
    cin>>s>>t;
    suffix_array<string>suf(s + "#" + t , true , false);
    int l1 = 0, l2 = 0 , len = 0;
    for(int i = 1;i < suf.suffix.size();i++) {
        if(suf.lcp[i] > len && suf.suffix[i - 1] < s.size() && suf.suffix[i] > s.size()) {
            len = suf.lcp[i];
            l1 = suf.suffix[i - 1];
            l2 = suf.suffix[i] - s.size() - 1;
        }else if(suf.lcp[i] > len && suf.suffix[i - 1] > s.size() && suf.suffix[i] < s.size()) {
            len = suf.lcp[i];
            l1 = suf.suffix[i];
            l2 = suf.suffix[i - 1] - s.size() - 1;
        }
    }
    cout<<l1<<" "<<l1 + len<<" "<<l2<<" "<<l2 + len<<line;
}
