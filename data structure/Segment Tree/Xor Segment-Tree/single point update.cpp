...
struct Seg {
    ...
    static vector<ll> mergeNode(const vector<ll>& L, const vector<ll>& R) {
        // merge 2
        int half = (int)L.size();
        vector<ll> res(half * 2);
        for (int i = 0; i < half; ++i) res[i] = merge(L[i] , R[i]);
        for (int i = 0; i < half; ++i) res[i + half] = merge(R[i] , L[i]);
        return res;
    }
    
    void build(int node, int b, int e, const vector<int>& a) {
        if (b == e) {
            t[node].assign(1, (b < (int)a.size() ? (ll)a[b] : 0LL));
            return;
        }
        int mid = (b + e) >> 1;
        int L = node << 1, R = L | 1;
    ...
    ll query(int node, int b, int e, int i, int j, int x, int layer = -2) {
        if (i > j || b > j || e < i) return 0LL;
