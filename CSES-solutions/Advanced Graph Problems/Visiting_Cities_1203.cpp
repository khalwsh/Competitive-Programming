...
Output
First print an integer kkk: the number of cities that are certainly in the route. After this, print the kkk cities sorted in increasing order.
Constraints
...
int main() {
    ...
    for (int i = 0;i < m;i++) {
        int u , v; cin >> u >> v;
        int w;cin >> w;
        e.emplace_back(array<int , 3>{u , v , w});
        adj[u].emplace_back(v , w);
        radj[v].emplace_back(u , w);
    }
    init(n);
    auto d1 = dijkstra(1 , adj);
    auto d2 = dijkstra(n , radj);
    for (auto &[u , v , w] : e) {
        if (d1[u] + w + d2[v] == d1[n]) {
            g[u].emplace_back(v);
        }
