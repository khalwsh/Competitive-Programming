...
There is an n×nn \times nn×n grid, and your task is to choose from each row and column some number of squares. How can you do that?
Input
...
The las line has nnn integers b1,b2,…,bnb_1,b_2,\ldots,b_nb1​,b2​,…,bn​: You must choose exactly bjb_jbj​ squares from the jjjth column.
Output
Print nnn lines describing which squares you choose (X means that you choose a square, . means that you don't choose it). You can print any valid solution.
...
int main() {
    ...
    auto ans = mf.solve(src, sink).second;
