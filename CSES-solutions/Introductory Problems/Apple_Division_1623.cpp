...
There are nnn apples with known weights. Your task is to divide the apples into two groups so that the difference between the weights of the groups is minimal.
Input
The first input line has an integer nnn: the number of apples.
The next line has nnn integers p1,p2,…,pnp_1,p_2,\dots,p_np1​,p2​,…,pn​: the weight of each apple.
Output
